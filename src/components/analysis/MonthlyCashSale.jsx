...
function MonthlyCashSale(){
    ...
    const totalCardShare = (cardShareData) => {
        if (cardShareData) {
            return Object.values(cardShareData).reduce((acc, value) => acc + value, 0);
        }
        return 0;
    }
    ...
    const fetchData = async (selectedDate) => {   
        try {
            ...
            const monthlySalesDate = `${year}년${month}월${day}일`;
            const res = await axios.get(`${baseURL}/cardOrCash?date=${monthlySalesDate}&choice=1`,
            {
                headers: {
    ...
    return(
        <div className="cashSales-content-wrap">
            <div className="cashSales-title page-title">월별 결제 보고서</div>
