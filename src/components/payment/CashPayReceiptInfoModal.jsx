...
function CashpayReceiptInfoModal({ paymentData, handlePaymentSuccess, closeModal }) {
    ...
    // 단일 상세 결제내역 불러오는 함수
    useEffect(() => {
        axios.get('http://54.180.60.149:3000/paymentOneList', { params: { receiptId : paymentData.receiptId }, 
    ...
    return (
      ...
      <div className="cashpayreceipt-1">
        ...
        {paymentdetail.param && (
          <>
          <div className="branch">[지점명] {paymentdetail.param.branchName}</div>
          <div className="branch-num">[사업자번호] 111-22-33333</div>
          <div className="branch-info">
            <div className="representative">[대표자] : {paymentdetail.param.representativeName}</div>
          </div>
          <div className="payment-date">[거래일시] {paymentdetail.param.purchasedAt}</div>
          </>
        )}
        </div>
        ...
        <div className="cashpayreceipt-body4">
          <div className="body4-top">
            ...
            <div className="body4-info2">{addComma(paymentdetail.param.price - (Math.round(paymentdetail.param.price*10/110)))}</div>
          </div>
          <div className="body4-top">
            <div className="body4-info1">부가세</div>
            <div className="body4-info2">{addComma(Math.round(paymentdetail.param.price*10/110))}</div>
          </div>   
        </div>
        ...
        <div className="cashpayreceipt-footer">
          ...
          <div className="payment-num">[승인번호] {paymentdetail.param.receiptId}</div>
        </div>
        </>
            )}
